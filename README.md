<div align="center">
  ...
  <p>
    ...
    <a href="CHANGELOG.md">
      <img src="https://img.shields.io/badge/changelog-1.0.3-orange" alt="Changelog">
    </a>
  </p>
</div>
...
The following backend API endpoints have been tested with sample prompts:

- `/api/generate` endpoint:
  - Command:
    ```
    curl -X POST -H "Content-Type: application/json" -d '{"prompt":"Test prompt for /api/generate"}' http://127.0.0.1:5001/api/generate
    ```
  - Sample Output:
    ```
    "response": "Okay here are a few examples of test prompts for a `/api/generate` endpoint ranging from simple to slightly more specific..."
    ```

- `/api/generate-with-thinking` endpoint:
  - Command:
    ```
    curl -X POST -H "Content-Type: application/json" -d '{"prompt":"Test prompt for /api/generate-with-thinking"}' http://127.0.0.1:5001/api/generate-with-thinking
    ```
  - Sample Output:
    ```
    "response": "Okay I will act as a test prompt for an API endpoint named `/api/generate-with-thinking`..."
    "thinking_summary": [
      "**Here's how I as the \"thinking\" API would approach this task:**..."
    ]
    ```
...
- `/api/text-to-speech` endpoint:
  - Command:
    ```
    curl -X POST -H "Content-Type: application/json" -d '{"text":"This is a test for text-to-speech API"}' http://127.0.0.1:5001/api/text-to-speech --output test_tts.mp3
    file test_tts.mp3
    ```
  - Sample Output:
    ```
    test_tts.mp3: MPEG ADTS layer III v2  64 kbps 24 kHz Monaural
    ```

- `/api/generate-image` endpoint:
  - Command:
    ```
    curl -X POST -H "Content-Type: application/json" -d '{"prompt":"Generate an image of a sunset over mountains"}' http://127.0.0.1:5001/api/generate-image --output test_image.png
    file test_image.png
    ```
  - Sample Output:
    ```
    test_image.png: PNG image data 1024 x 678 8-bit/color RGB non-interlaced
    ```
...
### APA Format
```
Das, N. (2025). AI Search with Gemini API (Version 1.0.3) [Computer software]. https://github.com/bniladridas/gemini-ai-search
```
