...
function handleUnifiedSearch() {
    ...
    if (!prompt) {
        showError('Type a role or prompt to continue.');
        return;
    }
...
function handleThinkingMode(prompt) {
    ...
    .then(data => {
        ...
        if (data.thinking_summary && data.thinking_summary.length > 0) {
            ...
            thinkingContainer.innerHTML = '<h4 style="color: #ff8930; margin-bottom: 0.5rem; font-weight: 600;">AI Thought Process</h4><p>No thinking summary available for this response. Try a different prompt or check the server logs for more information.</p>';
        }
